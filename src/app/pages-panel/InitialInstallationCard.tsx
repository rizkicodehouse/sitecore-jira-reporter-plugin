    ...
    >
      ...
      {phase === "idle" && (
        <button
          onClick={run}
          disabled={!canInstall}
          className="inline-flex items-center justify-center rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-white shadow-[0_10px_30px_-15px_rgba(110,63,255,0.6)] transition hover:bg-primary-600 active:bg-primary-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-400 disabled:cursor-not-allowed disabled:opacity-60"
      ...
      {(phase === "running" || phase === "success") && (
        <div className="flex flex-col gap-2" role="status">
          <div
            className="relative h-2 overflow-hidden rounded-full bg-primary-100"
            aria-hidden
          >
            <div
              className="absolute inset-y-0 left-0 rounded-full bg-primary shadow-[0_0_16px_rgba(110,63,255,0.45)] transition-[width] duration-500 ease-out"
